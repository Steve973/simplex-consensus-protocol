...
/**
 * Provides constants and utility methods for the Simplex consensus blockchain
 * protocol.
 */
@UtilityClass
public class SimplexConstants {
     ...
     *
     * @param iteration
     *            the iteration number of the block
     * @param <T>
     ...
     * @param quorumVotes
     *            the list of quorum votes
     * @return a notarized block with the given block and quorum votes
     */
    public static <T> NotarizedBlock<T> createNotarizedBlock(Block<T> block, List<Vote> quorumVotes) {
        return new NotarizedBlock<>(block, quorumVotes);
    }

    /**
     * Finalizes the current iteration by adding the notarized block to the
     * notarized blockchain.
     *
     * @param <T>
     *            the type of the transactions stored in the block
     * @param notarizedBlock
     *            the notarized block to be added
     * @param notarizedBlockchain
     *            the notarized blockchain to add the block to
     */
     ...
     * 
     * @param json
     *            the message details to convert
     * @return the {@link PeerInfo}
     * @throws JsonProcessingException
     *             if the details could not be converted
     */
    public static PeerInfo peerInfoFromJson(String json) throws JsonProcessingException {
     ...
     *            the type of the transactions stored in the block
     * @param proposal
     *            the Proposal object to be converted
    ...
    /**
     * Converts a SignedProposal object to a byte array using JSON serialization.
     *
     * @param <T>
     *            the type of the transactions stored in the block
     * @param proposal
     *            the SignedProposal object to be converted
     * @return a byte array representing the SignedProposal object
     * @throws JsonProcessingException
     *             if an error occurs during JSON serialization
     */
    public static <T> byte[] signedProposalToBytes(SignedProposal<T> proposal) throws JsonProcessingException {
        return JSON_MAPPER.writeValueAsBytes(proposal);
    }
     ...
     *
     * @param vote
     *            the Vote object to be converted
    ...
    /**
     * Converts a SignedVote object to a byte array using JSON serialization.
     *
     * @param vote
     *            the SignedVote object to be converted
     * @return a byte array representing the SignedVote object
     * @throws JsonProcessingException
     *             if an error occurs during JSON serialization
     */
    public static byte[] signedVoteToBytes(SignedVote vote) throws JsonProcessingException {
        return JSON_MAPPER.writeValueAsBytes(vote);
    }

    public static <T> T fromBytes(byte[] bytes) throws IOException {
        return JSON_MAPPER.readValue(bytes, new TypeReference<T>() {
        });
    }
